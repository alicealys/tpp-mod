...
namespace renderer_vars
{
	namespace
	{
		...
		bool get_fxaa_enabled()
		{
			...
			return *reinterpret_cast<int*>(SELECT_VALUE(0x142B79984, 0x142074DA4, 0x0, 0x0)) == 1;
		}
		...
		void volumetric_fog_manager_update_stub(utils::hook::assembler& a)
		{
			...
			a.jmp(SELECT_VALUE(0x1406BAF39, 0x14045BF99, 0x0, 0x0));

			a.bind(fog_enabled);
			a.jmp(SELECT_VALUE(0x1406BAFD8, 0x14045C038, 0x0, 0x0));
		}
	...
	public:
		void pre_load() override
		{
			...
			utils::hook::jump(SELECT_VALUE(0x1406BAF2C, 0x14045BF8C, 0x0, 0x0), utils::hook::assemble(volumetric_fog_manager_update_stub), true);
			utils::hook::jump(SELECT_VALUE(0x1402155B5, 0x140B1BCC5, 0x0, 0x0), utils::hook::assemble(gr_plugin_fxaa_main_exec_stub), true);

			gr_plugin_2d_main_exec_hook.create(SELECT_VALUE(0x1401C0AD0, 0x140AB09A0, 0x0, 0x0), gr_plugin_2d_main_exec_stub);
		}
	};
}
