...
namespace lui::renderer
{
		...
		void render_ui(game::fox::gr::dg::plugins::Draw2DRenderer* renderer)
		{
			...
			while (base_cmd->type != CMD_NONE && i < draw_list.count && reinterpret_cast<char*>(base_cmd) < draw_list.cmd_buffer_end)
			{
				...
				case CMD_DRAW_BOX:
				{
					...
					::renderer::draw_material(renderer, cmd->material, cmd->x, cmd->y, cmd->width, cmd->height, cmd->color, &cmd->params);
					break;
				}
				case CMD_DRAW_TEXT:
				{
					...
					if (cmd->artist_font)
					{
						::renderer::draw_text_artist(renderer, cmd->text, cmd->height, cmd->x, cmd->y, cmd->color,
							cmd->formatted, cmd->display_width, cmd->display_height, 0.f, 0.f, cmd->use_word_wrapping, -1, &cmd->params);
					}
					else
					{
						::renderer::draw_text(renderer, cmd->text, cmd->height, cmd->x, cmd->y, cmd->color,
							cmd->outline_color, cmd->formatted, cmd->display_width, cmd->display_height, 0.f, 0.f, cmd->use_word_wrapping, -1, &cmd->params);
					}
				...
				case CMD_DRAW_TEXT_CURSOR:
				{
					...
					::renderer::draw_text_with_cursor(renderer, cmd->text, cmd->cursor, cmd->height, cmd->x, cmd->y, cmd->color, cmd->outline_color, 
						cmd->formatted, cmd->display_width, &cmd->params);
				}
				}
	...
	void add_draw_material(const std::uint32_t material, float x, float y, float width, float height, float* color, ::renderer::params_t* params)
	{
		...
		std::memcpy(&cmd->params, params, sizeof(::renderer::params_t));
	...
	void add_draw_text(const char* text, float height, float x, float y, float* color, float* outline_color, bool formatted, 
		float display_width, float display_height, bool use_word_wrapping, bool artist_font, ::renderer::params_t* params)
	{
		std::lock_guard _0(draw_list.mutex);
	...
	void add_draw_text_with_cursor(const char* text, float height, float x, float y, float* color, float* outline_color, bool formatted,
		float display_width, int cursor, ::renderer::params_t* params)
	{
		std::lock_guard _0(draw_list.mutex);
