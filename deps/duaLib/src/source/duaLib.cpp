...
int main() {
	...
	while(true){
		s_ScePadData data = {};
		scePadRead(handle, &data, 0);
		printf("Buttons: %04X LStick: %d,%d RStick: %d,%d L2: %d R2: %d\n", data.bitmask_buttons, data.LeftStick.X, data.LeftStick.Y, data.RightStick.X, data.RightStick.Y, data.L2_Analog, data.R2_Analog);
	}
